...
<script>
	$(function() {
		var birthday = ${LOGIN_USER.birthday} + "";
		...
		$("#insertForm").submit(function(event) {
			...
			if(pwd.length < 8 || pwd.length > 20) {
				alert("비밀번호를 8자 이상 20자 이내로 입력하세요.");
				return false;
			}
			if(!birthdayyy) {
				alert("생년월일을 선택하세요.")
...
<div id="container ct" class="contBg">
	<div class="contWrap">
		<h2 class="pagTit">내 정보 수정</h2>
		<form id="insertForm" name="insertForm" method="post" action="updatemyinfo.do">
		<input type="hidden" value='${LOGIN_USER.no }' name="no"/>
		<input type="hidden" value='${LOGIN_USER.type }' name="type"/>
		<input type="hidden" value='${LOGIN_USER.name }' name="nameFirst"/>
			<div class="contBox">
				...
				<table class="tb-1 tbData" style="margin: 35px auto; width: 100%;">
					...
					<tbody>
								...
								<span class="radG fmMgL">
									<span class="radFm">
										<label for="type2" style="margin: 0;">
											<c:if test="${LOGIN_USER.type eq 'C' }">업체</c:if>
											<c:if test="${LOGIN_USER.type eq 'U' }">고객</c:if>
											</label>
									</span>
								</span>
							</td>
						...
						<tr>
							...
							<td class="fm">
								<input type="hidden" value="${LOGIN_USER.id }" class="ipt fw-24" title="아이디" id="id" name="id" maxlength="20">
								<span class="fmTxt-2">${LOGIN_USER.id }</span>
							</td>
						</tr>
						<tr>
							<th scope="row">
						...
						<tr>
							...
							<td class="fm">
								...
								<a href="#address" class="btnD btn-4 fmBtn" onclick="sample6_execDaumPostcode()">우편번호검색</a>
						...
						<c:if test="${LOGIN_USER.type eq 'U' }">
						<tr>
							<th scope="row" class="" style="text-align: center !important;">멤버십</th>
							<td class="fm">
								<label>${membership.grade }</label> <span class="dropdown"><button class="btnD btn-4 fmBtn" class="dropdown-toggle"
									data-toggle="dropdown" role="button" aria-haspopup="true"
									aria-expanded="false">멤버십 혜택</button>
									<div class="dropdown-menu" style="margin-left:100px; padding:13px; width:300px;">
										<strong>골드</strong><br/>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;조건: 누적 구매금액 300만원 이상<br/>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;혜택: 할인 10% 포인트 적립 5%<br/><br/>
										<strong>실버</strong><br/>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;조건: 누적 구매금액 200만원 이상<br/>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;혜택: 할인 5%   포인트 적립 5%<br/><br/>
										<strong>브론즈</strong><br/>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;조건: 누적 구매금액 100만원 이상<br/>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;혜택: 할인5% 포인트 적립 1%<br/><br/>
									</div></span>
							</td>
						</tr>
						<tr>
							...
							<td class="fm">
								<label>${LOGIN_USER.totalPoint } 포인트</label>
							</td>
						</tr>
						</c:if>
						<tr>
			...
			<div class="btnArea-1">
				<button type="submit" class="w3-btn w3-red" style="width: 126px !important; padding: 19px 32px; background-color: #e87a92 !important; text-decoration: none;">완료</button>
				<a href="dropuser.do?no=${LOGIN_USER.no }" class="w3-btn w3-red" style="width: 126px !important; padding: 19px 32px; background-color: #edadba !important; text-decoration: none;">탈퇴</a>
			</div>
		</form>
	</div>
</div>
