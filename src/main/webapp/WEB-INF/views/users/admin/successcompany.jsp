...
<%@ include file="../../include/mypagenav.jsp" %>
	<div class="container" style="font-family: '맑은 고딕'; width: 1216px !important;">
		...
		<ul class="nav nav-pills nav-justified">
		  <li><a href="companyadmin.do" style="background-color: white; color: #e87a92 !important; font-weight: bold;">모든 업체 리스트</a></li>
		  <li><a href="sbcompany.do" style="background-color: white; color: #e87a92 !important; font-weight: bold;">신청 업체 리스트</a></li>
		  <li class="active" style="background-color: white; font-weight: bold;"><a href="sccompany.do">등록된 업체 리스트</a></li>
		</ul>
...
$(function() {
	...
	$("#success-btn").click(function() {
		alert("해당 업체를 정지하였습니다.")
	})
	
	$.ajax({
		type : "GET",
		url : "../ajax/pageSubmitCompany.do",
		dataType : "json",
		data : {
			count : pagecount
			, status : 'Y'
		...
		success : function(results) {
			...
			$.each(results, function(index, company) {
				...
				row += "<tr>"
					...
					row += "<td><a href='updateCompany.do?cpno="+company.cpNo+"&st=F' id='success-btn'><button class='btn btn-warning btn-xs'>정지</button></a></td>"
				row += "</tr>"
			...
			success : function(results) {
				...
				$.each(results, function(index, company) {
					...
					row += "<tr>"
						...
						row += "<td>미승인</td>"
						row += "<td><a href='/final/detail.do?no="+company.cpNo+"'><button class='btn btn-default btn-xs'>상세</button></a></td>"
						row += "<td><a href='updateCompany.do?cpno="+company.cpNo+"st=Y' id='success-btn'><button class='btn btn-primary btn-xs'>승인</button></a></td>"
					row += "</tr>"
