...
.nav-pills>li {
  ...
  $(function() {
		...
		var num1 = telsplit[0];
		$("#resort_num1").val(num1);
		var num2 = telsplit[1];
		$("#resort_num2").val(num2);
		var num3 = telsplit[2];
		$("#resort_num3").val(num3);
		
		var address = '${company.address}';
		$('#resort_address').val(address);
		...
		$("#resortForm").submit(function(event) {
			var resortName = $("#resort_name").val().trim();
			var resortOwner = $("#resort_owner").val().trim();
			var resortContent = $("#resort_content").val().trim();
			var resortAddress = $("#resort_address").val().trim();
			...
			var resortNum1 = $("#resort_num1").val().trim();
			var resortNum2 = $("#resort_num2").val().trim();
			var resortNum3 = $("#resort_num3").val().trim();
      ...
      function sample6_execDaumPostcode() {
          new daum.Postcode({
              oncomplete: function(data) {
                  ...
                  document.getElementById('resort_address').value = data.zonecode; //5자리 새우편번호 사용
                  ...
                  var address_start = $("#detail_address1").val();
		            ...
		            if (status === daum.maps.services.Status.OK) {
		                ...
		                var infowindow = new daum.maps.InfoWindow({
		                    content: '<div style="width:150px;text-align:center;padding:6px 0;">'+$("#resort_name").val()+'</div>'
		                });
...
<div id="container ct" class="contBg" style="font-family: '맑은 고딕';">
	<div class="contWrap">
		...
		<form id="resortForm" name="resortForm" method="post" action="resortSubmit.do" enctype="multipart/form-data">
		<input type="hidden" name="cpNo" value="${cpNo }">
		<input type="hidden" id="order_start_lat" name="resortLocationLat">
		<input type="hidden" id="order_start_lon" name="resortLocationLng">
			<div class="contBox">
				...
				<table class="tb-1 tbData" style="margin: 35px auto; width: 100%;">
					...
					<tbody>
						<tr>
							<th scope="row">
								<span class="obligatory">
									<span class="blind">* </span>리조트명
								</span>
							</th>
							<td class="fm">
								<input type="text" class="ipt fw-24" value="${company.name }" id="resort_name" name="resortName" maxlength="20">
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td class="fm">
								<input type="text" class="ipt fw-24" value="${company.owner }" id="resort_owner" name="resortOwner" maxlength="20">
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td class="fm">
								<textarea rows="5" cols="50" id="resort_content" name="resortContent" maxlength="1000">${company.content }</textarea>
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td class="fm">
								<input type="file" value="${company.img }" id="resort_img" name="resortImg">
							</td>
						</tr>
						<tr>
							<th scope="row">
						...
						<tr>
							<th scope="row">
								<span class="obligatory">
									<span class="blind">* </span>리조트 연락처
								</span>
							</th>
							<td class="fm">
								<input type="text" class="ipt fw-3" title="전화번호 중간자리" id="resort_num1" name="resortNum1" value="" maxlength="4" />
								<span class="fmTxt-3">-</span>
								<input type="text" class="ipt fw-3" title="전화번호 중간자리" id="resort_num2" name="resortNum2" value="" maxlength="4" />
								<span class="fmTxt-3">-</span>
								<input type="text" class="ipt fw-3" title="전화번호 끝자리" id="resort_num3" name="resortNum3" value="" maxlength="4"/>
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td class="fm">
								<input type="text" class="ipt fw-7" title="우편번호" id="resort_address" name="resortAddress" />
								<a href="#resort_address" class="btnD btn-4 fmBtn" onclick="sample6_execDaumPostcode()">우편번호검색</a>
							...
							<script>
						        ...
						        if (navigator.geolocation) {
						            ...
						            navigator.geolocation.getCurrentPosition(function(position) {
						                ...
						                var locPosition = new daum.maps.LatLng(0, 0), // 마커가 표시될 위치를 geolocation으로 얻어온 좌표로 생성합니다
