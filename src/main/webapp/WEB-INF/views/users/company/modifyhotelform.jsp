...
.nav-pills>li {
  ...
  $(function() {
		...
		var addressDetail = '${company.addressDetail}';
		var addresssplit = addressDetail.split('/');
		$('#detail_address1').val(addresssplit[0]);
		$('#detail_address2').val(addresssplit[1]);
		
		$("#hotelForm").submit(function(event) {
			var hotelName = $("#hotel_name").val().trim();
			var hotelOwner = $("#hotel_owner").val().trim();
			var hotelContent = $("#hotel_content").val().trim();
			var hotelStar = $("#hotel_star").val().trim();
			var hotelAddress = $("#hotel_address").val().trim();
			...
			var hotelNum1 = $("#hotel_num1").val().trim();
			var hotelNum2 = $("#hotel_num2").val().trim();
			var hotelNum3 = $("#hotel_num3").val().trim();
      ...
      function sample6_execDaumPostcode() {
          new daum.Postcode({
              oncomplete: function(data) {
                  ...
                  document.getElementById('hotel_address').value = data.zonecode; //5자리 새우편번호 사용
                  ...
                  var address_start = $("#detail_address1").val();
		            ...
		            if (status === daum.maps.services.Status.OK) {
		                ...
		                var infowindow = new daum.maps.InfoWindow({
		                    content: '<div style="width:150px;text-align:center;padding:6px 0;">'+$("#hotel_name").val()+'</div>'
		                });
...
<div id="container ct" class="contBg" style="font-family: '맑은 고딕';">
	<div class="contWrap">
		<h2 class="pagTit">PARTY MIX 업체 수정</h2>
		<div class="pagTitSub" style="text-align: center; margin-bottom: 25px;">
			운영하시는 업체의 기본 정보를 수정 및 입력해주세요.
		</div>
		<div class="w3-container" style="padding: 0;">
			<ul class="nav nav-pills nav-justified" role="tablist">
				<li class="active">STEP1. 정보수정</li>
				<li>STEP2. 수정완료</li>
			</ul>
		</div>
		<form id="hotelForm" name="hotelForm" method="post" action="hotelSubmit.do" enctype="multipart/form-data">
		<input type="hidden" name="cpNo" value="${cpNo }">
		<input type="hidden" id="order_start_lat" name="hotelLocationLat">
		<input type="hidden" id="order_start_lon" name="hotelLocationLng">
			<div class="contBox">
				...
				<table class="tb-1 tbData" style="margin: 35px auto; width: 100%;">
					...
					<tbody>
						<tr>
							<th scope="row">
								<span class="obligatory">
									<span class="blind">* </span>호텔명
								</span>
							</th>
							<td class="fm">
								<input type="text" class="ipt fw-24" value="${company.name }" id="hotel_name" name="hotelName" maxlength="20">
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td class="fm">
								<input type="text" class="ipt fw-24" value="${company.owner }" id="hotel_owner" name="hotelOwner" maxlength="20">
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td class="fm">
								<textarea rows="5" cols="50" id="hotel_content" name="hotelContent" maxlength="1000">${company.contents }</textarea>
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td class="fm">
								<input type="file" value="${company.img }" id="hotel_img" name="hotelImg">
							</td>
						</tr>
						<tr>
							<th scope="row">
								<span class="obligatory">
									<span class="blind">* </span>등급
								</span>
							</th>
							<td class="fm">
								<input type="number" class="ipt fw-1" placeholder="1~5" value="${company.star }" id="hotel_star" name="hotelStar" min="1" max="5">
							</td>
						</tr>
						<tr>
							<th scope="row">
								<span class="obligatory">
									<span class="blind">* </span>관리자 연락처
								</span>
							</th>
							<td class="fm">
						...
						<tr>
							<th scope="row">
								<span class="obligatory">
									<span class="blind">* </span>호텔 연락처
								</span>
							</th>
							<td class="fm">
								<input type="text" class="ipt fw-3" title="전화번호 중간자리" id="hotel_num1" name="hotelNum1" value="" maxlength="4" />
								<span class="fmTxt-3">-</span>
								<input type="text" class="ipt fw-3" title="전화번호 중간자리" id="hotel_num2" name="hotelNum2" value="" maxlength="4" />
								<span class="fmTxt-3">-</span>
								<input type="text" class="ipt fw-3" title="전화번호 끝자리" id="hotel_num3" name="hotelNum3" value="" maxlength="4"/>
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td class="fm">
								<input type="text" class="ipt fw-7" title="우편번호" id="hotel_address" name="hotelAddress" />
								<a href="#hotel_address" class="btnD btn-4 fmBtn" onclick="sample6_execDaumPostcode()">우편번호검색</a>
								<input type="text" class="ipt fw-8 fmMg-1" title="주소" id="detail_address1" name="detailAddress1" maxlength="100"/>
								<input type="text" class="ipt fw-8 fmMg-1" title="상세주소" id="detail_address2" name="detailAddress2" maxlength="100" onkeyup=""/>
							</td>
						</tr>
						<tr>
							<th scope="row">
						        ...
						        if (navigator.geolocation) {
						            ...
						            navigator.geolocation.getCurrentPosition(function(position) {
						                ...
						                var locPosition = new daum.maps.LatLng(lat, lon), // 마커가 표시될 위치를 geolocation으로 얻어온 좌표로 생성합니다
						                    message = '<div style="padding:5px;">'+$("#hotel_name").val()+'</div>'; // 인포윈도우에 표시될 내용입니다
			...
			<div class="btnArea-1">
				<button type="submit" class="w3-btn w3-red" style="width: 126px !important; padding: 19px 32px; background-color: #e87a92 !important; text-decoration: none;">수정완료</button>
				<a href="deletecompany.do?dno=${cpNo }" class="w3-btn w3-red" style="width: 126px !important; padding: 19px 32px; background-color: #e87a92 !important; text-decoration: none;">정지</a>
			</div>
		</form>
	</div>
</div>
