...
<div class="container" style="min-height:600px">	
		...
		<thead>
			<tr>
				...
				<th colspan="3" style="color:#d36179; padding-right: 0px;">
					<p>
	...
	<input type="hidden" name="cpNo" value="${no }"/>
...
<script type="text/javascript">
	$(function() {
		...
		$('#ajax-table').on('shown.bs.modal','[id^="refund-modal"]', function() {
			...
			$("#btn-refund-todo"+reserveNo).on('click', function() {
				$('#refund-form'+reserveNo).find("input[name=reason]").val($(".reason").text());
		...
		function paging(data) {
			$.ajax({
				...
				type:'POST',
				url:'ajax/companyReserveList.do',
				dataType:'json',
				...
				success:function(reserves){
					...
					if (reserves.length) {
						$.each(reserves, function(index, reserve) {
							...
							row += '<tr>';
							row += '<th>예약자 이름</th>';
							row += '<td>' + reserve.user.name + '</td>'
							row += '</tr>';
							row += '<tr>';
							if (reserve.status == "결제") {
								row += '<th>요청사항</th>';
								if (reserve.request == null) {
									reserve.request = "없음";
								}
								row += '<td>' + reserve.request + '</td>';
							}
							if (reserve.status == "환불 요청") {
								row += '<th>환불사유</th>';
								if (reserve.reason == null) {
									reserve.reason = "없음";
								}
								row += '<td class="reason">' + reserve.reason + '</td>';
							}
							row += '</tr>';
							row += '<tr>';
							row += '<th>예약자 연락처</th>';
							row += '<td>';
							row += 'PHONE : ' + reserve.user.phone + '<br/>';
							row += 'EMAIL : ' + reserve.user.email;
							row += '</td>';
							row += '</tr>';
							row += '</thead>';
							row += '</table>';
							...
							if (reserve.difDay && reserve.status == "환불 요청") {
								...
								row += '</label>';
								row += '<input type="text" class="form-control" name="refund-price" readonly value="'
										+ numeral(reserve.refundPrice).format('0,0') + '원" style="text-align: right"/>';
								
							} 
							if (reserve.difDay && reserve.status == "결제") {
								row += '<input type="text" class="form-control" name="refund-price" readonly value="' 
										+ numeral(reserve.price).format('0,0') + '원" style="text-align: right"/>';
							} 
							row += '</div>';
							row += '</form>';
							row += '</div>';
							...
							row += '취소 하기</button>';
							row += '<button type="button" class="btn btn-default" data-dismiss="modal">돌아가기</button>';
							row += '<form id="refund-form' + reserve.reserveNo + '" action="company-refund.do">';
							row += '<input type="hidden" name="userNo" value="'+ reserve.userNo +'"/>';
							...
							row += '<input type="hidden" name="no" value="' + $("input[name=cpNo]").val() +'"/>';
							row += '<input type="hidden" name="type" value="' + $("input[name=type]").val() +'"/>';
							if (reserve.difDay && reserve.status == "환불 요청" ) {
								row += '<input type="hidden" name="refundPrice" value="'+ reserve.refundPrice + '"/>';
							}
						 	if (reserve.difDay && reserve.status == "결제") {
						 		row += '<input type="hidden" name="refundPrice" value="'+ reserve.price +'"/>';
						 	} 
						...
						row += '<td colspan="6"><div class="pagTitSub1"><div class="text-block"><h4 style="font-size: 50px !important; width:550px;">내역이 없습니다.</h4></div></div></td>';
						row += '</tr>';
					}
					$("#ajax-table").empty();
		...
		function newPage(data) {
			$.ajax({
				contentType:"application/json;charset=UTF-8",
				type:'POST',
				url:'ajax/companyReserveListCount.do',
				dataType:'text',
				data:JSON.stringify(data),
